...
class BaseAgent:
    ...
    max_attempts: int = 1
    def run(self, example: QAExample, use_mock: bool = True) -> RunRecord:
        ...
        for attempt_id in range(1, self.max_attempts + 1):
            answer, actor_tokens, actor_latency = actor_answer(
                example, attempt_id, self.agent_type, reflection_memory, use_mock=use_mock
            )
            judge, eval_tokens, eval_latency = evaluator(example, answer, use_mock=use_mock)
            ...
            if self.agent_type == "reflexion" and attempt_id < self.max_attempts and judge.score != 1:
                ref_entry, ref_tokens, ref_latency = reflector(
                    example, attempt_id, judge, use_mock=use_mock
                )
                reflections.append(ref_entry)
                reflection_memory.append(
                    f"Attempt {attempt_id} failed. Reason: {ref_entry.failure_reason}. "
                    f"Lesson: {ref_entry.lesson}. Next strategy: {ref_entry.next_strategy}"
                )
